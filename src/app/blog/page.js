          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {filteredArticles.map((art) => (
              <div
                ...
                className="group border border-slate-200/85 dark:border-slate-850 bg-slate-50/20 dark:bg-slate-900/20 rounded-2xl p-6 flex flex-col justify-between hover:border-indigo-500/40 hover:shadow-md transition-all duration-300 cursor-pointer"
              >
                <div>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-slate-850">
                  <div className="flex items-center gap-2">
                    <div className="h-6 w-6 rounded-full bg-slate-200 dark:bg-slate-850 flex items-center justify-center text-[10px] text-slate-600 dark:text-slate-400 font-bold uppercase">
                      {art.author[0]}
                    </div>
                    <div>
                      <span className="block text-[9px] font-bold text-slate-800 dark:text-slate-350">{art.author}</span>
                      <span className="block text-[8px] text-slate-400">{art.role}</span>
                    </div>
                  </div>
                  <span className="text-[9px] font-bold text-slate-450 dark:text-slate-500">{art.date}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {readingArticle && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
          <div className="relative w-full max-w-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden shadow-2xl z-10 flex flex-col max-h-[80vh]">
            ...
            {/* Modal Body */}
            <div className="p-6 overflow-y-auto flex flex-col gap-4 text-xs leading-relaxed text-slate-655 dark:text-slate-350">
              <div className="flex items-center gap-4 text-[10px] text-slate-450 mb-2 border-b border-slate-100 dark:border-slate-850 pb-4">
              ...
              <div className="mt-4 p-4 bg-slate-50 dark:bg-slate-950 rounded-2xl border border-slate-100 dark:border-slate-850 text-[10px] text-slate-500">
            ...
            <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/20 text-right">
              <button
                onClick={() => setReadingArticle(null)}
                className="px-5 py-2 border border-slate-200 dark:border-slate-850 rounded-xl text-[10px] font-bold uppercase tracking-wider hover:bg-slate-100 dark:hover:bg-slate-850"
              >
                Close Read
              </button>
            </div>
